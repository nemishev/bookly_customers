...
class AB_CustomerController extends AB_Controller {
    ...
    public function index() {
        ...
        wp_enqueue_script( 'ab-angularjs', '//ajax.googleapis.com/ajax/libs/angularjs/1.0.6/angular.min.js' );
        wp_enqueue_script( 'ab-angularui', '//cdnjs.cloudflare.com/ajax/libs/angular-ui/0.4.0/angular-ui.min.js' );
        wp_enqueue_script( 'ab-ng-sanitize', '//ajax.googleapis.com/ajax/libs/angularjs/1.0.6/angular-sanitize.js' );
    ...
    public function getCustomerData( $query ) {
        ...
        if ( count( $query ) ) {
            foreach( $query as $num => $customer_data ) {
                ...
                // get Amount of Payments via PayPal
                $query[ $num ]->payments = $wpdb->get_var("
                    SELECT SUM(t.amount) AS payments FROM
                       ( SELECT
                            p.created,
                            p.type,
                            c.id AS customer_id,
                            c.name AS customer,
                            r.*
                          FROM ab_payment_appointment r
                          INNER JOIN ab_payment p ON p.id = r.payment_id
                          INNER JOIN ab_customer c ON c.id = p.customer_id
                          INNER JOIN ab_appointment a ON r.appointment_id = a.id
                          WHERE p.type = 'paypal'
                       ) AS t WHERE customer_id = {$customer_data->id}
                ");
                if ( $query[ $num ]->payments ) {
                    $query[ $num ]->payments = AB_CommonUtils::formatPrice( $query[ $num ]->payments );
                } else {
